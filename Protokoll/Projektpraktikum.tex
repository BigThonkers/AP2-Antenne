...
\begin{figure}[h]
	...
	\centering
>>>>>>> 75b91438c12d8f51c6c10b4cccdd565e23f34a2c
\end{figure}
\FloatBarrier
\par 
Als nächstes nutzen wir die Python-Funktion curvfit \cite{curvescipy} Funktion um eine Funktion mit dem erwarteten $\frac{1}{d^{2}}$ Verlauf zu finden. Dazu nutzen wir:
\begin{equation}
f(d) = a + \frac{b}{d^{2}}
\label{f}
\end{equation}	
Hierbei sind $a$ und $b$ freie Parameter, welche curvfit an die Messwerte anpasst und $d$ der Abstand zur Sendeantenne. Zusätzlich wählen wir nun wieder eine logarithmische Skala für die $y$-Achse, um Messwerte, welche gegen Null gehen besser unterscheiden zu können. Dies ergibt (wieder mit den Messwerten aus dem großen Hörsaal) den Graphen Abbildung \ref{Abstand-bsp}.

\begin{figure}[h]
	
	\includegraphics[scale=0.5]{Bilder/Abstand-GrH}
	\centering
	\caption{Gemessenen Intensität in Abhängigkeit vom Abstand $d$. Auf der logarithmischen $y$-Achse ist die empfangene Intensität in Watt aufgetragen und auf der $x$-Achse der Abstand zur Sendeantenne in cm. Zudem wurden Fehler auf beiden Achsen eingezeichnet, jedoch sind die Fehler auf der $x$-Achse zu klein um sie zu sehen. Außerdem wurde mittels der curffit \cite{curvescipy} Funktion $f(d)$  \eqref{f} an die Messwerte angelegt.}
	\label{Abstand-bsp}
\end{figure}
